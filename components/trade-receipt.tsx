...
export function TradeReceipt({ receipt, onClosePosition, closing }: { receipt: TradeReceiptType; onClosePosition?: () => void; closing?: boolean }) {
  return (
    <section className="trade-receipt" aria-live="polite">
      <div className="receipt-heading"><span><Check size={17} weight="bold" /></span><div><p>Fill receipt</p><small>{receipt.venue.toUpperCase()} · {receipt.intent.toUpperCase()}</small></div></div>
      <dl>
        ...
        <div><dt>Order ID</dt><dd>{receipt.orderId ?? receipt.digest ?? 'Pending verification'}</dd></div>
        <div><dt>Attribution state</dt><dd>{receipt.attributionStatus.replaceAll('-', ' ')}</dd></div>
      </dl>
      <div className="receipt-actions">
        {onClosePosition && <button className="secondary-action" onClick={onClosePosition} disabled={closing}>{closing ? 'Closing…' : `Close ${receipt.venue.toUpperCase()} fade`}</button>}
